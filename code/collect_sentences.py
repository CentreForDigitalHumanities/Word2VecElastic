...
class ESCollector():
    '''
    Fetches text data from Elasticsearch.
    '''
    def __init__(
        self,
        corpus_name: str,
    ...
    def get_es_body(self) -> Dict:
        min_date = str(self.start_year)+"-01-01"
        max_date = str(self.end_year)+"-12-31"
        body = {
            "query": {
                "bool": {
                    "filter": [
...
class DataCollector():
    ...
    def get_pickle_filename(self, year):
        ...
        return os.path.join(self.source_directory, '{}-{}.pkl'.format(self.corpus_name, year))
    ...
    def get_sentences_for_year(self, year):
        ...
        collector = ESCollector(self.corpus_name, year, year)
